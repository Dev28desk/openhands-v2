...
# Makefile for DeskDev.ai project
...
start-backend:
	...
	@poetry run uvicorn deskdev.server.listen:app --host $(BACKEND_HOST) --port $(BACKEND_PORT) --reload --reload-exclude "./workspace"
...
_run_setup:
	...
	@poetry run uvicorn deskdev.server.listen:app --host $(BACKEND_HOST) --port $(BACKEND_PORT) &
...
help:
	...
	@echo "  $(GREEN)setup-config$(RESET)        - Setup the configuration for DeskDev.ai by providing LLM API key,"
	@echo "                        LLM Model name, and workspace directory."
	@echo "  $(GREEN)start-backend$(RESET)       - Start the backend server for the DeskDev.ai project."
	@echo "  $(GREEN)start-frontend$(RESET)      - Start the frontend server for the DeskDev.ai project."
	@echo "  $(GREEN)run$(RESET)                 - Run the DeskDev.ai application, starting both backend and frontend servers."
	@echo "                        Backend Log file will be stored in the 'logs' directory."
	@echo "  $(GREEN)docker-dev$(RESET)          - Build and run the DeskDev.ai application in Docker."
	@echo "  $(GREEN)docker-run$(RESET)          - Run the DeskDev.ai application, starting both backend and frontend servers in Docker."
	@echo "  $(GREEN)help$(RESET)                - Display this help message, providing information on available targets."

# Phony targets
.PHONY: build check-dependencies check-system check-python check-npm check-nodejs check-docker check-poetry install-python-dependencies install-frontend-dependencies install-pre-commit-hooks lint-backend lint-frontend lint test-frontend test build-frontend start-backend start-frontend _run_setup run run-wsl setup-config setup-config-prompts setup-config-basic deskdev-cloud-run docker-dev docker-run clean help
.PHONY: kind
